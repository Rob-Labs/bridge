...
const log_file = fs.createWriteStream(__dirname + "/ropsten_listener.log", {
  flags: "a",
});
...
const init = async () => {
  let lastBlockCkecked = fs.readFileSync("ropsten_last_block.txt").toString();

  const currentBlockHeight = await ropsten.web3.eth.getBlockNumber();
  ...
  if (currentBlockHeight > lastBlockCkecked) {
    ...
    fs.writeFileSync(
      __dirname + "/ropsten_last_block.txt",
