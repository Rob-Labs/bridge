...
const log_file = fs.createWriteStream(__dirname + "/eth_listener.log", {
  flags: "a",
});
...
const init = async () => {
  let lastBlockCkecked = fs.readFileSync("eth_last_block.txt").toString();

  const currentBlockHeight = await eth.web3.eth.getBlockNumber();
