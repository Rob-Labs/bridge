...
const log_file = fs.createWriteStream(__dirname + "/ropsten.log", {
  flags: "a",
});
  ...
  try {
    tx.send({
      ...
      .on("transactionHash", function (hash) {
        tempTxHash = hash;
        console.log(
          `Sending Redeem Tx ${data.transactionHash} to Ropsten blockchain network ${tempTxHash}, waiting for confirmation...`
        );
      })
      .on("receipt", (receipt) => {
        console.log(
      ...
      .on("error", function (error, receipt) {
        fs.writeFileSync(
          __dirname + "/ropsten_error.result",
...
RedeemTxQueue.process("ropsten", performRedeem);
