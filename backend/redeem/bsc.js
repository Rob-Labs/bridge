...
const log_file = fs.createWriteStream(__dirname + "/bsc.log", {
  flags: "a",
});
  ...
  try {
    tx.send({
      ...
      .on("transactionHash", function (hash) {
        tempTxHash = hash;
        console.log(
          `Sending Redeem Tx ${data.transactionHash} to BSC blockchain network ${tempTxHash}, waiting for confirmation...`
        );
      })
      .on("receipt", (receipt) => {
        console.log(
...
RedeemTxQueue.process("bsc", performRedeem);
