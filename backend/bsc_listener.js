...
const log_file = fs.createWriteStream(__dirname + "/bsc_listener.log", {
  flags: "a",
});
...
const init = async () => {
  let lastBlockCkecked = fs.readFileSync("bsc_last_block.txt").toString();

  const currentBlockHeight = await bsc.web3.eth.getBlockNumber();
